...
from src.models.transformer_model import GraphTransformer, AlignmentMLP, WeightScheduler
from src.alignment.pca_utils import prepare_and_cache_embeddings
...
class MarkovBridge(pl.LightningModule):
    def __init__(
        ...
        # Save only basic hyperparameters to avoid YAML serialization issues
        basic_hparams = {
            ...
            'fix_product_nodes': fix_product_nodes,
            'loss_type': loss_type,
        }
        self.save_hyperparameters(basic_hparams)
        ...
        # Initialize alignment components (64->128->128->64)
        self.alignment_mlp1 = AlignmentMLP(
            input_dim=hidden_dims['dy'], hidden_dims=[128, 128], output_dim=64)
        self.alignment_mlp2 = AlignmentMLP(
            input_dim=hidden_dims['dy'], hidden_dims=[128, 128], output_dim=64)
        self.weight_scheduler = WeightScheduler(
            max_steps=diffusion_steps, final_w=0.6)

        # Prepare PCA(whitened)->L2 embeddings for alignment (train/val)
        embeds = prepare_and_cache_embeddings('embeddings', n_components=64, force_recompute=False)
        self.prod_embeddings_train_64 = embeds['prod_train']
        self.react_embeddings_train_64 = embeds['react_train']
        self.prod_embeddings_val_64 = embeds['prod_val']
        self.react_embeddings_val_64 = embeds['react_val']

    def compute_alignment_loss(self, layer2_y, layer4_y, batch_indices, current_step):
        """
        Compute alignment loss using cosine similarity between MLP outputs and embeddings.

        Args:
            layer2_y: Output from layer 2 (for product alignment)
            layer4_y: Output from layer 4 (for reactant alignment)
            batch_indices: Indices of samples in the batch
            current_step: Current diffusion step for weight scheduling

        Returns:
            tuple: (mlp1_loss, mlp2_loss, total_alignment_loss, w)
        """
        # Ensure indices on CPU for CPU-stored targets
        batch_indices_cpu = batch_indices.detach().to('cpu').long().view(-1)

        # Get dynamic weights
        mlp1_weight, mlp2_weight, w = self.weight_scheduler.get_weights(
            current_step)

        # Get corresponding PCA(64d, whitened, L2) embeddings per split
        if self.training:
            prod_emb = self.prod_embeddings_train_64[batch_indices_cpu].to(layer2_y.device)
            react_emb = self.react_embeddings_train_64[batch_indices_cpu].to(layer4_y.device)
        else:
            prod_emb = self.prod_embeddings_val_64[batch_indices_cpu].to(layer2_y.device)
            react_emb = self.react_embeddings_val_64[batch_indices_cpu].to(layer4_y.device)

        # Apply MLP networks (output L2-normalized)
        mlp1_output = F.normalize(self.alignment_mlp1(layer2_y), dim=-1, eps=1e-8)
        mlp2_output = F.normalize(self.alignment_mlp2(layer4_y), dim=-1, eps=1e-8)

        # Compute cosine similarity losses
        mlp1_loss = 1 - \
            F.cosine_similarity(mlp1_output, prod_emb, dim=-1).mean()
        mlp2_loss = 1 - \
            F.cosine_similarity(mlp2_output, react_emb, dim=-1).mean()

        # Weighted combination
        total_alignment_loss = mlp1_weight * mlp1_loss + mlp2_weight * mlp2_loss

        return mlp1_loss, mlp2_loss, total_alignment_loss, w

    def configure_optimizers(self):
        # Include MLP parameters in optimization
        params = list(self.model.parameters(
        )) + list(self.alignment_mlp1.parameters()) + list(self.alignment_mlp2.parameters())
    ...
    def process_and_forward(self, data, return_intermediate_outputs=False):
        ...
        if return_intermediate_outputs:
            pred, intermediate_outputs = self.forward(
                noisy_data, extra_data, node_mask, return_intermediate_outputs=True)
        else:
        ...
        if return_intermediate_outputs:
            return reactants, product, pred, node_mask, noisy_data, context, intermediate_outputs
        return reactants, product, pred, node_mask, noisy_data, context

    def training_step(self, data, i):
        # Get intermediate outputs for alignment
        reactants, product, pred, node_mask, noisy_data, _, intermediate_outputs = self.process_and_forward(
            data, return_intermediate_outputs=True)

        if self.loss_type == 'vlb':
            main_loss_dict = self.compute_training_VLB(
                reactants=reactants,
                pred=pred,
                node_mask=node_mask,
            ...
            main_loss_dict = self.compute_training_CE_loss_and_metrics(
                reactants=reactants, pred=pred, i=i)

        main_loss = main_loss_dict['loss']

        # Compute alignment loss if intermediate outputs are available
        if intermediate_outputs and 'layer_2_y' in intermediate_outputs and 'layer_4_y' in intermediate_outputs:
            # Get batch indices for embedding lookup
            batch_indices = data.idx

            # Get current diffusion step for weight scheduling
            current_step = noisy_data['t_int'].squeeze(
            ).max().item()  # Use max step in batch

            # Compute alignment loss
            mlp1_loss, mlp2_loss, total_alignment_loss, w = self.compute_alignment_loss(
                layer2_y=intermediate_outputs['layer_2_y'],
                layer4_y=intermediate_outputs['layer_4_y'],
                batch_indices=batch_indices,
                current_step=current_step
            )

            # Add alignment loss with 0.5 weight relative to X loss weight
            alignment_weight = 0.5
            total_loss = main_loss + alignment_weight * total_alignment_loss

            # Log alignment losses
            if i % self.log_every_steps == 0:
                self.log('alignment/mlp1_loss', mlp1_loss.detach())
                self.log('alignment/mlp2_loss', mlp2_loss.detach())
                self.log('alignment/total_loss', total_alignment_loss.detach())
                self.log('alignment/weight_w', w)
                self.log('alignment/total_combined_loss', total_loss.detach())
    ...
    def validation_step(self, data, i):
        ...
        try:
            if intermediate_outputs and 'layer_2_y' in intermediate_outputs and 'layer_4_y' in intermediate_outputs:
                batch_indices = data.idx
                current_step = noisy_data['t_int'].squeeze().max().item()
                mlp1_loss, mlp2_loss, total_alignment_loss, w = self.compute_alignment_loss(
                    layer2_y=intermediate_outputs['layer_2_y'],
                    layer4_y=intermediate_outputs['layer_4_y'],
                    batch_indices=batch_indices,
                ...
                if i % self.log_every_steps == 0:
                    self.log('val_alignment/mlp1_loss', mlp1_loss.detach())
                    self.log('val_alignment/mlp2_loss', mlp2_loss.detach())
    ...
    def save_accuracy_checkpoints(self):
        """Save checkpoints based on accuracy metrics after sampling"""
        try:
            # Get current metrics from the logged values
            logged_metrics = self.trainer.logged_metrics

            if 'top_1_accuracy' in logged_metrics:
                top_1_acc = logged_metrics['top_1_accuracy'].item()
                top_1_dir = os.path.join(
                    self.checkpoints_dir, 'top_1_accuracy')
                os.makedirs(top_1_dir, exist_ok=True)

                checkpoint_path = os.path.join(
                    top_1_dir, f'{self.current_epoch:03d}_{top_1_acc:.3f}.ckpt')
                self.trainer.save_checkpoint(checkpoint_path)

            if 'top_5_accuracy' in logged_metrics:
                top_5_acc = logged_metrics['top_5_accuracy'].item()
                top_5_dir = os.path.join(
                    self.checkpoints_dir, 'top_5_accuracy')
                os.makedirs(top_5_dir, exist_ok=True)

                checkpoint_path = os.path.join(
                    top_5_dir, f'{self.current_epoch:03d}_{top_5_acc:.3f}.ckpt')
                self.trainer.save_checkpoint(checkpoint_path)

        except Exception as e:
            print(f"Warning: Could not save accuracy-based checkpoints: {e}")
    ...
    def forward(self, noisy_data, extra_data, node_mask, return_intermediate_outputs=False):
        X = torch.cat((noisy_data['X_t'], extra_data.X), dim=2).float()
        E = torch.cat((noisy_data['E_t'], extra_data.E), dim=3).float()
        y = torch.hstack((noisy_data['y_t'], extra_data.y)).float()
        return self.model(X, E, y, node_mask, return_intermediate_outputs=return_intermediate_outputs)
    ...
    def sample_chain(
        ...
        product_discrete, _ = utils.to_dense(
            data.p_x, data.p_edge_index, data.p_edge_attr, data.batch)
